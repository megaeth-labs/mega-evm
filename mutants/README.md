...
- **infrastructure** — the code that implements the test gates, which the helper exclusion removes from the production scope: the scenario runner (`crates/mega-evm/src/test_utils/scenario.rs`).
  Mutated against `mega-evm`'s tests.
...
| `.cargo/mutants-infra.toml` | Config for the infrastructure scope (no helper exclusion). |
