...
impl DynamicGasCost {
    ...
    pub fn sstore_set_gas<DB: SaltEnv>(
        ...
        let bucket_id = DB::bucket_id_for_slot(address, key);
        let multiplier = self.load_bucket_cost_multiplier(db, bucket_id)?;
    ...
    pub fn new_account_gas<DB: SaltEnv>(
        ...
        // increase the gas cost according to the bucket capacity
        let bucket_id = DB::bucket_id_for_account(address);
        let multiplier = self.load_bucket_cost_multiplier(db, bucket_id)?;
    ...
    fn load_bucket_cost_multiplier<DB: SaltEnv>(
        ...
        match self.bucket_cost_mulitipers.entry(bucket_id) {
            ...
            Entry::Vacant(vacant_entry) => {
                let capacity = db.get_bucket_capacity(bucket_id)?;
