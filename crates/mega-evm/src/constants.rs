...
pub mod mini_rex {
    ...
    /// for code deposit during contract creation. `2_000_000 / 32 = 62500`. This is charged on
    /// top of the `CODEDEPOSIT` gas cost of standard EVM.
    pub const CODEDEPOSIT_ADDITIONAL_GAS: u64 = 62500 - super::equivalence::CODEDEPOSIT;
    /// The gas cost for `LOGDATA` for the `MINI_REX` spec, i.e., gas cost per byte for log data.
    pub const LOG_DATA_GAS: u64 = 800;
    /// The gas cost for `LOGTOPIC` for the `MINI_REX` spec, i.e., gas cost per topic for log.
    pub const LOG_TOPIC_GAS: u64 = 37500;
    ...
    pub const CALLDATA_ADDITIONAL_GAS: u64 = 400;
