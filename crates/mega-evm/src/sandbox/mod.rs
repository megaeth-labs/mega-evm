...
//! # Sandbox hook
//!
//! Nested sandbox execution is otherwise invisible to a parent inspector. A
//! [`SandboxInspector`] attaches through [`crate::MegaContext::set_keyless_sandbox_hook`]
...
//! context satisfy both through the blanket impl. Lifecycle events follow the same
//! slot rule, so a hook with one impl per env sees a sandbox entirely on one impl.
//!
//! [`SandboxInspector`]: inspector::SandboxInspector
//! [`SandboxEndOutcome::state_applied`]: inspector::SandboxEndOutcome::state_applied
//!
