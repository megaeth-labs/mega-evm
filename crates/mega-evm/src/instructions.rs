...
/// `LOG` opcode implementation modified from `revm` to support data bomb.
///
/// # Difference from the standard EVM
///
/// The only difference is that after all the logic of the [standard
/// EVM](revm::interpreter::instructions::host::log), we check if the total data size exceeds the
/// limit.
///
/// # Assumptions
///
...
///   initially, and doubles as the corresponding SALT bucket capacity doubles. The actual gas cost
///   is provided by [`crate::GasCostOracle::sstore_set_gas`].
/// - The data bomb. When the total amount of data generated by the current transaction exceeds the
///   limit, the transaction will error and halt (consuming all remaining gas).
...
/// # Difference from the standard EVM
///
/// - The gas cost for transferring value to an empty account (i.e., creating a new account) is
