...
use mega_evm::{
    ...
    ACCOUNT_INFO_WRITE_SIZE, BASE_TX_SIZE, LIMIT_CONTROL_ADDRESS, STORAGE_SLOT_WRITE_SIZE,
};
use revm::{
    bytecode::opcode::*,
    context::{
...
fn test_intrinsic_data_size_overflow_with_inspector_early_return() {
    ...
    let mut tx = MegaTransaction(OpTransaction::new(default_tx_builder(CALLEE).build_fill()));
    tx.enveloped_tx = Some(Bytes::new());
