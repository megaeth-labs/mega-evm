...
With `--trace.call.with-log`, a sandbox frame shows a log only when the receipt holds it: before REX5 an empty-code deployment drops the constructor's logs, and so does the trace.
